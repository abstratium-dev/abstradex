...
public class AccountService {
    ...
    public Optional<@NonNull Demo> findCredentialByUsername(String username) {
        var query = em.createQuery("SELECT c FROM Credential c WHERE c.username = :username", Demo.class);
        query.setParameter("username", username);
        return query.getResultStream().findFirst();
    }

    public Optional<@NonNull Demo> findCredentialByAccountId(String accountId) {
        var query = em.createQuery("SELECT c FROM Credential c WHERE c.accountId = :accountId", Demo.class);
    ...
    public Optional<@NonNull Account> authenticate(String username, String password) {
        Optional<Demo> credentialOpt = findCredentialByUsername(username);
        if (credentialOpt.isEmpty()) {
            return Optional.empty();
        }

        Demo credential = credentialOpt.get();
    ...
    public boolean updatePassword(String accountId, String oldPassword, String newPassword) {
        Optional<Demo> credentialOpt = findCredentialByAccountId(accountId);
        if (credentialOpt.isEmpty()) {
